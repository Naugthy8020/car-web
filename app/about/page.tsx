...
import { useDarkMode } from '../context/DarkModeContext'; // ダークモードのコンテキストをインポート

const AboutPage = () => {
  const { isDarkMode } = useDarkMode(); // ダークモードの状態を取得

  return (
    <div className={`max-w-4xl mx-auto px-4 py-12 ${isDarkMode ? 'bg-gray-800 text-white' : 'bg-white text-gray-800'}`}>
      {/* 会社名とロゴ */}
      <div className={`flex items-center mb-8 border-b ${isDarkMode ? 'border-gray-500' : 'border-gray-300'} pb-4`}>
        <Image
      ...
      {/* 会社のビジョン */}
      <section className={`mb-8 border-b ${isDarkMode ? 'border-gray-500' : 'border-gray-300'} pb-8`}>
      ...
      {/* SDGsの取り組み */}
      <section className={`mb-8 ${isDarkMode ? 'bg-green-800 text-white' : 'bg-green-100 text-gray-800'} p-6 rounded-lg border-b ${isDarkMode ? 'border-gray-500' : 'border-gray-300'} pb-8`}>
        ...
        <ul className={`list-disc pl-6 text-lg ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
      ...
      {/* お客様の声 */}
      <section className={`border-b ${isDarkMode ? 'border-gray-500' : 'border-gray-300'} pb-8`}>
      ...
      <section className="mt-12 text-sm" id="policy">
        <h2 className={`text-2xl font-semibold mb-4 ${isDarkMode ? 'text-white' : 'text-gray-800'}`}>会社情報・ポリシー</h2>

        <div className="mb-6">
          <h3 className={`text-lg font-semibold mb-2 ${isDarkMode ? 'text-white' : 'text-gray-800'}`}>会社概要</h3>
        ...
        <div className="mb-6">
          <h3 className={`text-lg font-semibold mb-2 ${isDarkMode ? 'text-white' : 'text-gray-800'}`}>個人情報保護方針</h3>
        ...
        <div>
          <h3 className={`text-lg font-semibold mb-2 ${isDarkMode ? 'text-white' : 'text-gray-800'}`}>サービスに関するポリシー</h3>
        ...
        <p className={`mt-8 text-center ${isDarkMode ? 'text-gray-500' : 'text-gray-400'}`}>&copy; {new Date().getFullYear()} Car Inc. All rights reserved.</p>
      </section>
    </div>
  );
};
